...
def fetch_lecture_data(self, taskRequest: LectureDetailsByScholarRequest):
    ...
    signatures = [
        run_pipeline_worker.s(
            PipelineRequest(lecture_details_by_scholar=lecture_response)
            )
...
def run_pipeline_worker(self, request: PipelineRequest) -> PipelineResponse | None:
    ...
    try:
        ...
        try:
            print(f"Task {self.request.id} failed, retrying in {self.default_retry_delay}s. Attempt {self.request.retries + 1}")
...
def validate_and_(request: LectureDetailsByScholarResponse):
